...
const AlertDialog = ({ isOpen, onClose, title, message, type = 'info' }) => {
    ...
    return (
        ...
        >
            <div className={`flex items-start gap-4 p-4 rounded-xl border ${config.bgColor} ${config.borderColor}`}>
                <div className="flex-shrink-0">
                    {config.icon}
                </div>
                <p className={`${config.textColor} font-medium leading-relaxed`}>{message}</p>
            </div>
        </Modal>
    );
};
